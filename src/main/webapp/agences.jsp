...
<div class="container-fluid">
   ...
   <table id="tablebd" class="table table-borderd table-hover table-sptriped mt-4" style=" display : inline block">
    ...
    <tbody>
        <%
        String[] valeur = (String []) request.getAttribute("nbrmac");
        String[] noms = (String[]) request.getAttribute("noms");
        String[] localisation = (String[]) request.getAttribute("location");
        if (noms != null) {
            for (int j = 0; j < noms.length; j++) {
            %>
            <tr class="selectable-row"  data-row-index="<%= j %>">
                <td onclick= "macliste(<%= j %>)"><input type="hidden" id="agname<%= j %>" value="<%= noms[j] %>"><%= noms[j] %></td>
                <td onclick= "macliste(<%= j %>)"><input type="hidden" id="agname<%= j %>" value="<%= noms[j] %>"><%= valeur[j] %></td>
            </tr>
            <%
            }}
            %>
...
<!-- Ajout machine -->
 				...
 				<div class="col-md-3">
  				...
  				<div class="col-md-4">
    				<label for="inputAddress" class="form-label">Observation</label>
  					<textarea class="form-control" id="Pblm" name="Pblm" rows="3" ></textarea>  				
  				</div>
  				<div class="col-md-4">
 ...
 <div id="lstable" class="modal" style= "display :none" >
  <div class="modal-content">
    <span class="fermer" onclick="closeModal()">&times;</span>
    <h2>Liste materiel </h2>
    <!-- Le tableau sera ajouté ici -->
    <table id="tablels"></table>
  </div>
</div>
    </div>
    </div>
