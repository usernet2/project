...
<div class="container-fluid">
    <div class="row flex-nowrap">
        ...
        <!-- SEARCH BAR	 -->
        <div class="col py-3">
        <form action="Materielcrud" class="row" method="GET">
        ...
        <!-- FILTRE -->
        <form action="Materielcrud" class="row" method="GET" id="filtre" style=" display : none">
        <div class="col-sm-2">
  		...
  		<div class="col-sm-2">
    		<select name="Categorie" id="inputState" class="form-select">
        ...
        <div class="col-sm-2 "><button name="action" value="filter"  type="submit" class="btn btn-primary">FILTRER</button></div>
        </form> <br> 
        <!-- TABLEAU -->
         <table id="tabletransfert" class="table table-borderd table-hover table-sptriped mt-4" style=" display : none" ></table>
    ...
    <thead>
        ...
        <th id ="hbox" style=" display : none"></th>
            ...
            %>
            <th>
                <button id="addRowButton" class="btn btn-link-black" onclick="toggleMForm()">
            ...
            <tr class="selectable-row"  data-row-index="<%= j %>">
             <td id="box"  style=" display :none "  ><input type="checkbox" name="selectedRow" value="<%= noms[0][j] %>" checked></td>
                ...
                <td>
                    ...
                    <button onclick="afficherPopupM(<%= j %>)" class="btn btn-secondary">Update</button>
                </td>
                <td>
                    <form class="" action="Materielcrud" method="GET">
...
<!-- Ajout machine -->
   ...
   <div id="addcmpprinter" style=" display :none" >
				...
				<form action="Composant" class="row g-3" method = "GET">
  				<div class="col-md-4">
    				<label for="inputEmail4" class="form-label">N° Serie Composant</label>
   					<input type="text" class="form-control" name="N°SerC" id="N°SerC" required>
				</div>
  				<div class="col-md-4">
    				<label for="inputPassword4" class="form-label">References</label>
    				<input type="text" class="form-control" name="RefC" id="RefC" required>
 				</div>
  				<div class="col-md-4">
    				<label for="inputAddress" class="form-label">Designation</label>
  				...
  				<div class="col-md-4">
   		 			<label for="inputAddress2" class="form-label">Date Entree</label>
    				<input type="date" class="form-control" name="DateC" id="DateEC" required>
 	 			</div>
 				<div class="col-12">
    ...
    <!-- Update machine -->
       <div id="popupM" class="popup" >
        <div class="popup-content">
            <span onclick="masquerPopupM()" class="close" id="closeButton">&times;</span>
            <h2>Modifier Machine</h2>
            <form id="popupForm" class="row g-3" action="Materielcrud" method="GET">
              	<div class="col-md-4">
    				<label  class="form-label">N° Serie Machine</label>
   					<input type="text" class="form-control" id="UNSerM" name="UNSerM" required>
   					<input type="hidden" class="form-control" id="AUNSerM" name="AUNSerM" >
				</div>
  				<div class="col-md-4">
    				<label  class="form-label">References</label>
    				<input type="text" class="form-control" id="URefM" name="URefM" required>
 				</div>
  				<div class="col-md-4">
    				<label for="inputAddress" class="form-label">Designation</label>
   	 				<select name="UDropdes" id="UDropdes" class="form-select">
      					<option selected>PC portable</option>
      					<option >PC U.C</option>
      					<option >IMPRIMANTE Laser</option>
      					<option >IMPRIMANTE Epson</option>
  				...
  				<div class="col-md-4">
    				...
    				<input type="date" class="form-control" id="UDateEM" name="UDateEM" required>
 	 			</div>
  				<div class="col-md-4">
    				<label  class="form-label">Etat</label>
    				<select id="UEtatM" name="UEtatM" class="form-select">
      					<option>Active</option>
      					<option selected >Stock</option>
      					<option>Reparation</option>
    				</select>
  				...
  				<div class="col-md-4">
  	    	<label for="inputState" class="form-label">Agences</label>
    		<select name="Ulocation" id="locationupt" class="form-select">
          <%  String[] agence = (String[]) request.getAttribute("agences");
          if (agence != null) {
              for (int j = 0; j < agence.length; j++) {
                  String option = agence[j];
              %>
              <option><%= option %></option>
              <%
              }}
          %> 
 				...
 				<div class="col-12">
    				<button type="submit" value="Update" name="action" class="btn btn-outline-success">Enregistrer</button>
   					<button type="submit" class="btn btn-outline-danger">Annuler</button>
  				</div>
            </form>
        </div>
 				...
 				<div class="col-md-3">
  				...
  				<div class="col-md-4">
    				<label for="inputAddress" class="form-label">Observation</label>
  					<textarea class="form-control" id="Pblm" name="Pblm" rows="3" required></textarea>  				
  				</div>
  				<div class="col-md-4">
    				<label for="inputAddress" class="form-label">Solution</label>
  					<textarea class="form-control" id="solt" name="solt" rows="3"></textarea>  				
  				</div>
  				<div class="col-md-3">
    ...
    <!-- Info Materiel -->
 <%
String[] infom = (String[]) request.getAttribute("info");
if (infom != null) {
    for (int i = 0; i < infom.length; i++) {
%>
    <input type="hidden" id="ref_<%= i %>" value="<%= infom[i] %>">
<%
    }
}
%>
    <div id="reppc" class="popup" >
        <div class="popup-content">
            ...
            <h2>Info</h2>
            <form id="popupForm" class="row g-3" action="Info" method="GET">
                <div class="col-auto">
    				<label  class="form-label">N° Serie Machine</label>
   					<input type="text" class="form-control" id="Imac" name="infomac"  disabled required>
   					<input type="hidden" class="form-control" id="I_mac_h" name="infomach"   >
   					<input type ="hidden" id="dateinfo" name ="datei">
				</div>
  				<div class="col-auto">
    				<label  class="form-label">References</label>
    				<input type="text" class="form-control" id="Iref" name="inforef" disabled required>
 				</div> 
  				<div class="col-md-4">
    				<label  class="form-label">CM</label>
   	 				<input type="text" class="form-control" id="IRcm" name="Rcm"  disabled required>
  				</div>
  				<div class="col-md-4">
   		 			<label class="form-label">CPU</label>
    				<input type="text" class="form-control" id="IRcpu" name="Rcpu"  disabled required>
 	 			</div>
  				<div class="col-md-4">
    				<label  class="form-label">RAM</label>
    				<input type="text" class="form-control" id="IRram" name="Rram"  disabled required>
  				</div>
  				<div class="col-md-4">
    				<label  class="form-label">Memoires</label>
    				<input type="text" class="form-control" id="IRrom" name="Rrom"  disabled required>
  				</div>
  				<div class="col-md-4">
    				<label  class="form-label">Alimentation</label>
    				<input type="text" class="form-control" id="IRalim" name="Ralim"  disabled required>
  				</div>
 				<div class="col-12">
    				<a onclick="reparation()" class="btn btn-outline-secondary">Reparer</a>
   					<button type="submit" class="btn btn-outline-danger">Annuler</button>
  				</div>
            </form>
        </div>
    ...
    <div id="repprinter" class="popup" >
        <div class="popup-content">
            ...
            <form id="popupForm" class="row g-3" action="Reparation" method="GET">
                <div class="col-auto">
    				<label  class="form-label">N° Serie Machine</label>
   					<input type="text" class="form-control" id="Imac" name="infomac" disabled>
   					<input type="hidden" class="form-control" id="I_mac_h" name="infomach"   >
   					<input type ="hidden" id="dateinfo" name ="datei">
				</div>
  				<div class="col-auto">
    				<label  class="form-label">References</label>
    				<input type="text" class="form-control" id="Iref" name="inforef" disabled >
 				</div> 
  				<div class="col-md-4">
    				<label  class="form-label">Toner</label>
